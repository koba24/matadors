...
// Matadorscoin: optional setting to unlock wallet for block minting only;
...
// Matadorscoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Matadorscoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // Matadorscoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Matadorscoin: not avatar mode
                        {
...
// Matadorscoin: create coin stake transaction
...
// Matadorscoin: check 'spent' consistency between wallet and txindex
// Matadorscoin: fix wallet spent state according to txindex
...
// Matadorscoin: disable transaction (only for coinstake)
